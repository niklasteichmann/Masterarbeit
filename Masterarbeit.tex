...
\title{ToDo Schematische 2D Visualisierung von RNA-Protein Interaktionen\xspace}
...
Als Vergleichsdarstellung, und um eine Vorstellung \"uber die Struktur der Datens\"atze zu vermitteln, wurde f\"ur jeden Datensatz eine Visualisierung der Tensoren als Ellipsoide erzeugt. Die Ellipsoide werden abh\"angig von den Eigenwerten des jeweiligen Tensors unterschiedlich dargestellt. Die L\"ange der Achsen eines Ellipsoids entspricht den Eigenwerten und die l\"angste Achse zeigt in Richtung des dazugeh\"origen Eigenvektors.
...
Abb. \ref{SinglePointEllipsoids} zeigt eine Darstellung der Tensoren als Ellipsoide. Die L\"angen der Halbachsen entsprechen den Betr\"agen der  Eigenwerte. \ref{SinglePointInvariantSpace} zeigt die Darstellung des Datensatzes im Invariantenraum in gr\"un mit Interaktionsfl\"achen links und rechts davon. Das DVR des Objekts ist in Abb. \ref{SinglePointObject} zu sehen.

In Abb. \ref{SinglePointInteraction} wurden verschiedene Invariantenbereiche ausgew\"ahlt. Verwendet wurde der K-Invariantensatz. Bei \ref{SinglePointInvariant1} und \ref{SinglePointObject1} sind die Bereiche mit niedriger ersten Invariante ausgeblendet, in \ref{SinglePointInvariant2} und \ref{SinglePointObject2} die Bereiche mit niedriger zweiter Invariante und in \ref{SinglePointInvariant3} und \ref{SinglePointObject3} sind nur noch Bereiche mit sehr hoher dritter Invariante zu sehen. 
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Die mechanische Spannung des Single Point Load-Datensatzes mit ausgew\"ahlten Invariantenbereichen. (a) und (b) zeigen Bereiche mit hohem $K_1$ (Spur), (c) und (d) Bereiche mit hohem $K_2$ (Deviatornorm), (e) und (f) Bereiche mit hohem $K_3$ (Modus).}
...
Als zweites Beispiel wurde das Ergebnis einer thermo-mechanischen Simulation, durchgef\"uhrt von Dr. Thomas Nagel, verwendet, die auch in der Arbeit von Friztsch benutzt wurde \cite[S.15, 45 ff.]{fritzsch2016continuousScatterplot}. Fritzsch beschreibt den Datensatz wie folgt:

``Die Metallscheibe liegt flach auf und ist entlang ihres \"au{\ss}eren Rands fixiert. Zwecks Umformung wirkt von oben ein hoher Druck ein. Zus\"atzlich wird sie von unten erw\"armt und von oben gek\"uhlt.''\cite[S.~15]{fritzsch2016contibuousScatterplot}
...
Abb. \ref{MetalDiskStressEllipsoids} zeigt die Spannungstensoren der Metallscheibe in Form von Ellipsoiden. Pro Punkt wird ein Ellipsoid gezeichnet, die sich aber \"uberschneiden k\"onnen, was die Interpretation erschwert. Erkennbar ist jedoch, dass die Ellipsoide am \"au{\ss}eren Rand der Scheibe eher l\"anglich und die am inneren Rand eher scheibenf\"ormig sind. Das bedeutet, dass die Tensoren am \"ausseren Rand einen einzelnen Eigenwert mit hohem und zwei Eigenwerte mit kleinem Betrag besitzen, wogegen am inneren Rand zwei Eigenwerte hohe und nur einer einen kleinen Betrag hat. In beiden Bereichen ist hohe Anisotropie zu erwarten. Gleichzeitig nimmt die Gr\"o{\ss}e der Ellipsoiden im Bild von unten links nach oben rechts zu. Dies ist darauf zur\"uckzuf\"uhren, dass nach oben rechts hin negative Eigenwerte mit immer gr\"o{\ss}erem Betrag vorkommen.

Die Darstellungen der Invariantenbereiche (Abb. \ref{MetalDiskStressInteraction}) lassen weitere Interpretationen zu. In Abb. \ref{MetalDiskStressObject1} ist zu sehen, dass die Spur im oberen rechten Teil des Scheibenausschnitts am kleinsten ist, da die negativen Eigenwerte dort den gr\"o{\ss}ten Betrag haben.  Abb. \ref{MetalDiskStressObject2} zeigt, dass die Norm des Deviators am inneren und \"au{\ss}eren Rand der Scheibe gro{\ss} und im Bereich dazwischen eher klein ist, was auf hohe Anisotropie hinweist. Der Modus ist am inneren Rand der Scheibe gro{\ss} und am inneren klein (Abb. \ref{MetalDiskStressInvariant2}).Das bedeutet, dass die Anisotropie am inneren Rand eher planar und am \"ausseren Rand eher linear ist.
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Die mechanische Spannung des Metallscheiben-Datensatzes mit ausgew\"ahlten Invariantenbereichen. (a) und (b) zeigen Bereiche mit niedrigem $K_1$ (Spur), (c) und (d) Bereiche mit niedrigem $K_2$ (Deviatornorm), (e) und (f) Bereiche mit hohem $K_3$ (Modus).}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Die Verformung des Metallscheiben-Datensatzes mit ausgew\"ahlten Invariantenbereichen. (a) und (b) zeigen Bereiche mit hohem $K_1$ (Spur), (c) und (d) Bereiche mit niedrigem $K_2$ (Deviatornorm), (e) und (f) Bereiche mit hohem $K_3$ (Modus).}
...
\begin{figure}
	...
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[height=2.5cm]{pictures/results/Bremshebel_R_InvariantSpace.png}
		\subcaption{}
		\label{BremshebelObjectR}
	\end{subfigure}
	\caption{Darstellung des Bremshebel-Datensatzes: (a) als Ellipsoide, (b) als DVR des Objekts und (c) im Invariantenraum, berechnet mit den R-Invarianten.}
	\label{BremshebelR}
\end{figure}
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Die mechanische Spannung des Bremshebel-Datensatzes mit ausgew\"ahlten Invariantenbereichen. (a) und (b) zeigen Bereiche mit hohem $K_1$ (Spur), (c) und (d) Bereiche mit hohem $K_2$ (Deviatornorm), (e) und (f) Bereiche mit niedrigem $K_3$ (Modus).}
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Die mechanische Spannung des Bremshebel-Datensatzes mit ausgew\"ahlten Invariantenbereichen. (a) und (b) zeigen Bereiche mit hohem $R_1$ (Frobeniusnorm), (c) und (d) Bereiche mit hohem $R_2$ (fraktionale Anisotropie), (e) und (f) Bereiche mit niedrigem $R_3$ (Modus).}
...
\begin{figure}
	\begin{minipage}[c]{0.66\textwidth}
		\begin{subfigure}{\textwidth}
			\centering
			\includegraphics[height=3cm]{pictures/results/Plate3D_Ellipsoids.png}
			\subcaption{}
			\label{Plate3DIObject}
		\end{subfigure}	
		\hspace*{\fill}
		\begin{subfigure}{\textwidth}
			\centering
			\includegraphics[height=3cm]{pictures/results/Plate3D_I_Object.png}
			\subcaption{}
		\end{subfigure}
	\end{minipage}
	\hspace*{\fill}
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[height=6cm]{pictures/results/Plate3D_I_InvariantSpace.png}
		\subcaption{}
	\end{subfigure}

	\caption{Darstellung des Metallplatten-Datensatzes: (a) als Ellipsoide, (b) als DVR des Objekts und (c) im Invariantenraum, berechnet mit den I-Invarianten.}
	\label{Plate3DI}
\end{figure}

\subsection{Metallplatte}

Es ist m\"oglich auch mit den I und J-Invariantens\"atzen Visualisierungen zu erzeugen, obwohl die Darstellung im zylindrischen Koordinatensystem f\"ur diese nur wenig Sinn macht. Um dies zu demonstrieren, wurde ein weiterer Datensatz gew\"ahlt.

Der letzte Datensatz bla bla ToDo Felix

Da die Platte d\"unn ist, ist auch die Darstellung im Invariantenraum d\"unn, weshalb die einzelnen Teilbereiche schwer zu erkennen sein k\"onnen.

Die Ellipsoide sind an der oberen Kante der Platte am gr\"o{\ss}ten (Abb. \ref{Plate3DIObject}), dies ist jedoch wieder auf negative Eigenwerte mit hohem Betrag zur\"uckzuf\"uhren. An den ToDo Punkten sind sie eher l\"anglich und werden mit zunehmender Entfernung kugelf\"ormiger.

Auff\"allig sind ebenfalls die sehr kleinen Ellipsoide entlang zweier B\"ogen in der Mitte der Platte und gro{\ss}e Ellipsoide an den unteren Ecken. 

Es wurde jeweils eine Visualisierung f\"ur den I und J Satz erzeugt. Da die Interpretation von $I_2, I_3, J_2$ und $J_3$ schwierig ist, beschr\"anken sich die Texte auf die Auswertung von $I_1$ und die Beschreibung der Bereiche der anderen Invarianten.

\subsubsection{I-Invarianten}
Die Spur ist, wie nach der Auswertung der Ellipsoiddarstellung zu erwarten war, an der oberen Kante der Platte am niedrigsten (siehe Abb. \ref{Plate3DIInvariant1} und \ref{Plate3DIObject1}). Interessant ist dabei, dass an den ToDo Punkten der ausgew\"ahlte Bereich am d\"unnsten ist. Im gleichen Bereich treten die h\"ochsten Werte f\"ur $I_2$ auf (Abb. \ref{Plate3DJInvariant2} und \ref{Plate3DJObject2}). 

$I_3$ dagegen ist im unteren Bereich und an den ToDo Punkten am h\"ochsten (Abb. \ref{Plate3DIInvariant3} und \ref{Plate3DIObject3}).
...
\begin{figure}
	...
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[height=6cm]{pictures/results/Plate3D_J_InvariantSpace.png}
		\subcaption{}
	\end{subfigure}
	
	\caption{Darstellung des Bremshebel-Datensatzes: (a) als Ellipsoide, (b) als DVR des Objekts und (c) im Invariantenraum, berechnet mit den J-Invarianten.}
	\label{Plate3DJ}
\end{figure}

\subsubsection{J-Invarianten}
Da $J_1$ immer gleich 0 ist, wurde stattdessen $I_1$ verwendet. Dadurch sind die ersten Invarianten beider S\"atze gleich, genauso wie die dargestellten Bereiche (Abb. \ref{Plate3DJObject1} und \ref{Plate3DJInvariant1}). $J_2$ dagegen ist nahe der ToDo Punkte und an den unteren beiden Ecken am h\"ochsten. Auch $I_3$ ist bei den ToDo Punkten am h\"ochsten.


\begin{figure}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/Plate3D_I_InvariantSpace1.png}
		\subcaption{}
		\label{Plate3DIInvariant1}
	\end{subfigure}
	\hspace*{\fill}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=3cm]{pictures/results/Plate3D_I_Object1.png}
		\subcaption{}
		\label{Plate3DIObject1}
	\end{subfigure}
	\medskip
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/Plate3D_I_InvariantSpace2.png}
		\subcaption{}
		\label{Plate3DIInvariant2}
	\end{subfigure}
	\hspace*{\fill}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=3cm]{pictures/results/Plate3D_I_Object2.png}
		\subcaption{}
		\label{Plate3DIObject2}
	\end{subfigure}
	\medskip
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/Plate3D_I_InvariantSpace3.png}
		\subcaption{}
		\label{Plate3DIInvariant3}
	\end{subfigure}
	\hspace*{\fill}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=3cm]{pictures/results/Plate3D_I_Object3.png}
		\subcaption{}
		\label{Plate3DIObject3}
	\end{subfigure}
	\caption{Die mechanische Spannung des Metallplatten-Datensatzes mit ausgew\"ahlten Invariantenbereichen. (a) und (b) zeigen Bereiche mit niedrigem $I_1$ (Spur), (c) und (d) Bereiche mit hohem $I_2$ (Summe der Hauptminoren), (e) und (f) Bereiche mit hohem $I_3$ (Determinante).}
	\label{Plate3DIInteraction}
\end{figure}
...
\begin{figure}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/Plate3D_J_InvariantSpace1.png}
		\subcaption{}
		\label{Plate3DJInvariant1}
	\end{subfigure}
	\hspace*{\fill}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=3cm]{pictures/results/Plate3D_J_Object1.png}
		\subcaption{}
		\label{Plate3DJObject1}
	\end{subfigure}
	\medskip
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/Plate3D_J_InvariantSpace2.png}
		\subcaption{}
		\label{Plate3DJInvariant2}
	\end{subfigure}
	\hspace*{\fill}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=3cm]{pictures/results/Plate3D_J_Object2.png}
		\subcaption{}
		\label{Plate3DJObject2}
	\end{subfigure}
	\medskip
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/Plate3D_J_InvariantSpace3.png}
		\subcaption{}
		\label{Plate3DJInvariant3}
	\end{subfigure}
	\hspace*{\fill}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=3cm]{pictures/results/Plate3D_J_Object3.png}
		\subcaption{}
		\label{Plate3DJObject3}
	\end{subfigure}
	\caption{Die mechanische Spannung des Metallplatten-Datensatzes mit ausgew\"ahlten Invariantenbereichen. (a) und (b) zeigen Bereiche mit niedrigem $I_1$ (Spur), (c) und (d) Bereiche mit hohem $J_2$ (Summe der Hauptminoren des Deviators), (e) und (f) Bereiche mit hohem $J_3$ (Determinante des Deviators).}
	\label{Plate3DJInteraction}
\end{figure}
...
Ein weiterer Ansatzpunkt f\"ur Verbesserungen w\"are das Voxelisierungsverfahren. Nicht nur dass die Voxelisierung einen gro{\ss}en Teil der Rechenzeit in Anspruch nimmt, es wird auch noch ein bewusster Fehler begangen. Die Dichte innerhalb eines Voxels entspricht im Moment stets der Summe der Tetraeder, die das Voxel schneiden, unabh\"angig davon, wie viel des Voxels vom Tetraeder eingenommen wird. Dadurch enth\"alt die 3D Textur insgesamt mehr Masse als das urspr\"ungliche Feld, was gegen einen Grundsatz des Continuous Scatterplottings verst\"o{\ss}t, die Masseerhaltung. Eine M\"oglichkeit diesen Fehler zu reduzieren oder sogar komplett zu vermeiden w\"are es, pro Voxel die Werte an mehreren Samplepoints zu berechnen und die Dichte auf den Mittelwert der Samplepunkte zu setzen. OpenGL bietet standardm\"a{\ss}ig eine Option dieses Supersampling zu implementieren, dies ist jedoch noch nicht in dieser Arbeit geschehen.
