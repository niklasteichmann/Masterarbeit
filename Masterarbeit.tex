...
Der I Invariantensatz ist jedoch nicht orthogonal. Dies wird ersichtlich, wenn man die Darstellung des I-Invariantensatzes durch die Eigenwerte $\lambda_1, \lambda_2, \lambda_3$ betrachtet:

\begin{itemize}
	\item $I_1(A) = \lambda_1 + \lambda_2 + \lambda_3$
	\item $I_2(A) = \lambda_1\lambda_2 + \lambda_1\lambda_3 + \lambda_2\lambda_3$
	\item $I_3(A) = \lambda_1\lambda_2\lambda_3$ 
\end{itemize}

Wenn zwei der Invarianten als positiv und fest angenommen werden, und die dritte steigt, so steigen auch alle drei Invarianten des I-Invariantensatzes, was die Bedingung der Orthogonalit\"at verletzt.
...
Ein DVR eines quaderf\"ormigen Ausschnitts des Halbraums ist in Abb. \ref{SinglePointObject} zu sehen. 

Abb. \ref{SinglePointEllipsoids} zeigt eine Darstellung der Tensoren als Ellipsoidglyphen. Deutlich erkennbar ist der Punkt an dem die Kraft einwirkt, dort sind die Glyphen am gr\"o{\ss}ten. Durch die \"Uberdeckung der Glyphen ist es schwer zu erkennen an welchen Punkten die Glyphen genau eingezeichnet sind, es scheint jedoch so als ob die Matrizen nahe der Oberfl\"ache stark planar anistrop sind, was durch flache runde Glyphen dargestellt ist. Weiter im Inneren ist die Form der Glyphen eher l\"anglich, was auf lineare Anisotropie hinweist. 

F\"ur die Visualisierung wurde der I-Invariantensatz gew\"ahlt. Abb. \ref{SinglePointInvariantSpace} zeigt die Form des Datensatzes im I-Invariantenraum. Da der I-Invariantensatz keiner der Invariantens\"atze ist, die sich f\"ur die Darstellung in zylindrischen Koordinaten eignen, wurden ein kartesischen Koordinatensystem und das dazugeh\"orende quadrische Interaktionswidget verwendet. 

Die Darstellung wurde gedreht, so dass die durchgef\"uhrten Interaktionen besser sichtbar werden. Durch die Drehung befindet sich der Koordinatenursprung an der vorderen linken unteren Ecke des Interaktionswidgets, erkennbar dadurch dass dort alle drei farbigen Kanten beginnen. Die rote Kante geht vom Koordinatenursprung aus nach hinten, in Richtung der steigenden ersten, die gelbe Kante nach oben in Richtung der steigenden zweiten und die blaue Kante nach links in Richtung der steigenden dritten Invariante. Die Dimensionen des Invariantenraumes wurde durch eine Option des Algorithmus so skaliert, dass die Bereiche 

Die Darstellung des Datensatzes im Invariantenraum 

Interdie Koordinaten der Tetraeder f\"ur das kartesische Koordinatensystem berechnet. Dementsprechend wird auch das quadrische Interaktionswidget angezeigt, dargestellt durch die
Abb. \ref{SinglePointInvariantSpace} zeigt die Darstellung des Datensatzes im Invariantenraum in gr\"un mit Interaktionswidgets links und rechts davon. Das DVR des Objekts ist in Abb. \ref{SinglePointObject} zu sehen.
...
\begin{figure}
	...
	\begin{subfigure}{0.25\textwidth}
		\centering
		\includegraphics[height=6cm]{pictures/results/MetalDisk/stress/MetalDisk_Stress_Ellipsoids.png}
		\subcaption{}
	...
	\end{subfigure}
	\caption{Der Metallscheiben-Datensatz: (a) DVR des Objektes, (b) Tensorglyphen der mechanischen Spannung und (c) Tensorglyphen der mechanischen Verformung.}
	\label{MetalDisk}
\end{figure}
...
Da die Metallscheibe spiegelsymmetrisch zur x- und y-Achse ist, gen\"ugt es, ein Viertel der Scheibe zu simulieren. Als Ergebnisse der Simulation wurden zwei Datens\"atze erzeugt, die jeweils die mechanische Spannung und die Verformung an den Punkten der Metallscheibe enthalten. Die Metallscheibe sowie Tensorglyphen der beiden Datens\"atze sind in Abb. \ref{MetalDisk} dargestellt. 

Beide Datens\"atze wurden von Fritzsch untersucht. Dabei wurden als Invarianten $I_1$ und $\sqrt{2J}$ verwendet. Da im dreidimensionalen gilt $I_1 = K_1$ und $\sqrt{2J_2} = K_2$ und die Tensoren beider Datens\"atze dreidimensional sind, ist im Folgenden stattdessen vom K-Invariantensatz die Rede. Um die Korrektheit der in der vorliegenden Arbeit implementierten Visualisiserung zu zeigen und die Ergebnisse mit denen von Fritzsch vergleichen zu k\"onnen, werden im Folgenden Visualisierungen des K-Invariantensatzes f\"ur mechanische Spannung und mechanische Verformung vorgestellt und mit den Visualisierungen von Fritzsch verglichen.

Um die visuelle Vergleichbarkeit zu erh\"ohen, wurden auch Bilder mit orthogonalen Projektionen erzeugt. Daf\"ur existiert im implementierten FAnToM Plugin noch keine Option. Da die erzeugten Bilder jedoch einige Vorteile bieten, zum Beispiel die M\"oglichkeit Positionen im Invariantenraum leichter abzulesen, k\"onnte diese Option in Zukunft noch implementiert werden. Die f\"ur die DVRs verwendete Transferfunktion wurde ebenfalls so gew\"ahlt, dass die \"Ahnlichkeit der Visualisierungen deutlich wird.

\begin{figure}
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[height=4cm]{pictures/results/MetalDisk/stress/MetalDisk_Stress_Fritzsch.png}
		\subcaption{}
		\label{MetalDiskStressFritzsch}
	\end{subfigure}
	\hspace*{\fill}
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[height=4cm]{pictures/results/MetalDisk/stress/MetalDisk_Stress_Orthogonal.png}
		\subcaption{}
		\label{MetalDiskStressOrthogonal}
	\end{subfigure}
	\hspace*{\fill}
	\begin{subfigure}{0.3\textwidth}
		\centering
		\includegraphics[height=4cm]{pictures/results/MetalDisk/stress/MetalDisk_Stress_Perspective.png}
		\subcaption{}
		\label{MetalDiskStressPerspective}
	\end{subfigure}
	\caption{Darstellungen des Invariantenraums des K-Invariantensatzes f\"ur den mechanischen Stress in der Simulation der Metallscheibe: (a) Die von Fritzsch vorgestellte Visualisierung \cite[S.~45]{fritzsch2016continuousScatterplot}, (b) ein DVR mit orthogonaler Projektion, (c) ein DVR mit perspektivischer Projektion.}
\end{figure}

\subsubsection{Spannung}
Abb. \ref{MetalDiskStressFritzsch} zeigt die von Fritzsch erzeugte Darstellung der f\"ur die Metallscheibe simulierten mechanischen Spannung. Die x-Achse entspricht dabei der Invariante $K_1$ und die y-Achse $K_2$. Der gleiche Datensatz ist in Abb. \ref{MetalDiskStressOrthogonal} als DVR mit orthogonaler Perspektive visualisiert, wobei die x-Achse wiederum $K_1$ und die y-Achse $K_2$ entspricht. Die z-Achse entspricht $K_3$, verschwindet jedoch durch die orthogonale Projektion, da die Blickrichtung der Kamera parallel zur z-Achse ist. In Abb. \ref{MetalDiskStressPerspective} ist derselbe Datensatz noch einmal dargstellt, jedoch mit perspektivischer Projektion.

Die \"Ahnlichkeit zwischen Abb. \ref{MetalDiskStressFritzsch} und \ref{MetalDiskStressOrthogonal} ist offensichtlich. Insbesondere der l\"angliche Bereich hoher Dichte unten mit seiner geschwungenen oberen Kante ist deutlich wiedererkennbar. 

Es gibt jedoch auch deutliche Unterschiede. Der obere Bereich in Abb. \ref{MetalDiskStressOrthogonal} ist deutlich heller als in Abb. \ref{MetalDiskStressFritzsch}. Dies ist darauf zur\"uckzuf\"uhren, dass die z-Koordinaten der Eckpunkte der Tetraeder in diesem Bereich sehr nah beieinander liegen, die Tetraeder also fast Fl\"achen parallel zur xz-Ebene sind. Dadurch schneiden die f\"ur bei der Rasterisierung verwendeten Fl\"achen nur wenige Tetraeder, was zu den komplett weissen Bereichen und den linienf\"ormigen Artefakten im DVR f\"uhrt. Eine m\"ogliche L\"osung dieses Problems w\"are, den Winkel der der Rasterisierung verwendeten Fl\"achen zur xy-Ebene einstellbar zu machen.

Ein weiterer erkennbarer Unterschied sind die verschiedenen F\"arbungen an den rechten Kanten der Darstellungen. Bei der Visualisierung von Fritzsch ist dieser Bereich sehr dunkel, im DVR dagegen in einem mittleren Grau. Der Grund daf\"ur liegt in den unterschiedlichen Berechnungen der Farbwerte. Der Algorithmus von Fritzsch addiert f\"ur jedes Pixel die darauf projezierten Dichtewerte der Tetraeder und berechnet aus dieser Summe den Farbwert des Pixels. Dies entspricht dem Vorgehen beim Erstellen von R\"iontgenbildern, wie in Abschnitt \ref{sec:DVR} beschrieben. Das implementierte DVR dagegen verwendet das Vorgehen zum Erzeugen transluzenter Bilder, das im selben Abschnitt beschrieben wird.

 
Abb. \ref{MetalDiskStressEllipsoids} zeigt die Spannungstensoren der Metallscheibe in Form von Ellipsoidglyphen. Erkennbar ist, dass die Glyphen am \"au{\ss}eren Rand der Scheibe eher l\"anglich und die am inneren Rand eher scheibenf\"ormig sind. Das bedeutet, dass die Tensoren am \"ausseren Rand einen einzelnen Eigenwert mit hohem und zwei Eigenwerte mit kleinem Betrag besitzen, wogegen am inneren Rand zwei Eigenwerte hohe und nur einer einen kleinen Betrag hat. In beiden Bereichen ist hohe Anisotropie zu erwarten. Gleichzeitig nimmt die Gr\"o{\ss}e der Glyphen im Bild von unten links nach oben rechts zu. Daraus ist erkennbar, dass der Betrag der Eigenwerte von unten links nach oben rechts zunimmt. Die Informationen \"uber das Vorzeichen der Eigenwerte geht jedoch in der Glyphendarstellung verloren. Deshalb l\"asst sich nicht ablesen, ob die hohen Eigenwertbetr\"age auf Ausdehnung oder Schrumpfen in diesem Bereich hinweist. 
...
\begin{figure}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/MetalDisk/stress/cylindrical/MetalDiskStress_InvariantSpace1_C.png}
		\subcaption{}
	...
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/MetalDisk/stress/MetalDiskStress_Object1.png}
		\subcaption{}
	...
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/MetalDisk/stress/cylindrical/MetalDiskStress_InvariantSpace2_C.png}
		\subcaption{}
	...
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/MetalDisk/stress/MetalDiskStress_Object2.png}
		\subcaption{}
	...
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/MetalDisk/stress/cylindrical/MetalDiskStress_InvariantSpace3_C.png}
		\subcaption{}
	...
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/MetalDisk/stress/MetalDiskStress_Object3.png}
		\subcaption{}
...
\begin{figure}
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/MetalDisk/strain/cylindrical/MetalDiskStrain_InvariantSpace1_C.png}
		\subcaption{}
	...
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/MetalDisk/strain/cylindrical/MetalDiskStrain_InvariantSpace2_C.png}
		\subcaption{}
	...
	\begin{subfigure}{0.49\textwidth}
		\centering
		\includegraphics[height=4.5cm]{pictures/results/MetalDisk/strain/cylindrical/MetalDiskStrain_InvariantSpace3_C.png}
		\subcaption{}
...
Die meisten im Kapitel \hyperref[sec:Verwandte]{Verwandte Arbeiten} erw\"ahnten klassischen Tensorfeldvisualisierungen verwenden die Eigenwerte und -vektoren um Tensoren darzustellen. Der Betrag eines einzelne Eigenwertes liefert ist jedoch oft nicht sehr aussagekr\"aftig, viel wichtiger ist das Verh\"altnis der Eigenwerte zueinander. Invarianten, wie sie vorgestellt wurden, liefern diese Informationen, weshalb sie in der Kontinuumsmechanik und der Untersuchung von Materialien verwendet werden.
...
Wie in den Vergleichen mit den Ergebnissen von Fritzsch erw\"ahnt ist die Implementierung einer Option zum Umschalten auf orthogonale Projektion im DVR eine m\"oglicherweise lohnenswerte Erweiterung. Auch Optionen zur Erstellung von R\"ontgenbildern, MIPs oder Isof\"achen sind vorstellbar.
