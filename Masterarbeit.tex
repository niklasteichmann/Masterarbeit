...
Wenn die zu einem Invariantensatz geh\"orenden Invarianten einer Matrix $M$ berechnet wurden, so stellen diese eine Repr\"asentation der basisunabh\"angigen Eigenschaften von $M$ dar. In dieser Arbeit wurden ausschlie{\ss}lich symmetrischen $3\times3$ Matrizen visualisiert, die aus bis zu 6 nicht-doppelten Werten bestehen k\"onnen. Die Beschreibung dieser 6 Werte durch die 3 Invarianten ist nicht ohne Informationsverlust m\"oglich. Die verlorenen 3 Werte an Information sind jedoch ausschlie{\ss}lich Informationen \"uber die Richtung der Eigenvektoren, was in vielen Anwendungsf\"allen nicht von Interesse ist. Somit ist die Darstellung einer Matrix durch ihre Invarianten ein geeigneter Ansatz, um die in Herausforderung 3 der Einleitung genannten Probleme zu l\"osen.

Welcher der Invariantens\"atze f\"ur einen bestimmten Datensatz verwendet wird, kann abh\"angig von der Dom\"ane ausgesucht werden, was in Hinblick auf Herausforderung 4 interessant ist. Die Wahl ist nat\"urlich abh\"angig von den schon genannten Interpretationen der Invarianten in den jeweiligen Dom\"anen.
...
An die Interaktionen wurden zwei weitere Anforderungen gestellt. 

Zum einen sollte die Art der Interaktion m\"oglist bei der Bearbeitung der in der Einleitung genannten Herausforderungen helfen. Dadurch, dass die Interaktionen Teile des Datensatzes ausblenden, helfen sie dabei, Herausforderung 2 zu l\"osen. Herausforderung 3, das intuitive Vermitteln von Informationen \"uber die Daten, ist eng verbunden mit dem 4. Kriterium von Leiner et al. und wird f\"ur beide Interaktionswidgets getrennt beschrieben.

Zum anderen sollten die Interaktionen so implementiert werden, dass die Auswirkung einer Interaktion mit so kurzer Verz\"ogerung wie m\"oglich angezeigt wird. Die Implementierung der Auswirkungen der Interaktionen ist innerhalb des Raycastings umgesetzt, wodurch der zus\"atzliche Aufwand sehr gering ist. Tats\"achlich wird die Laufzeit durch Interaktion mit dem quadrische Interaktionswidget generell und durch Interaktion mit der ersten Invariante beim zylindrischen Interaktionswidget reduziert, da die Bounding Box kleiner wird.
...
Die erzeugten 3D Texturen bestehen aus $400\times400\times400$ Voxeln im Invariantenraum und $600\times600\times100$ Voxeln im Ortsraum. Wie schon zuvor wurde die Anzahl der Voxel im Ortsraum an die Form des Objektes angepasst, um Speicher zu sparen.

Die Rasterisierung in die 3D Texturen ben\"otigte insgesamt etwa 3,3 Sekunden, die ben\"otigte Rechenzeit f\"ur ein einzelnes Bild betrug im Durchschnitt ca. 0,1 Sekunden.
...
\begin{figure}
	...
	\begin{subfigure}{0.49\textwidth}
		...
		\label{NodelObject1}
	\end{subfigure}
	\caption{Der Metallkomponenten-Datensatz, eingschr\"ankt auf einen kleinen Bereich mit hohen Werten der ersten und zweiten Invariante: (a) Darstellung im Invariantenraum und (b) Darstellung im Ortsraum.}
...
Im Gegensatz zu z.B. der Darstellung von Tensoren als Glyphen kommt es nicht zu \"Uberdeckungen. Durch das DVR kann das Innere der Datens\"atze leicht betrachtet werden. Ausserdem k\"onnen die Werte der Tensoren sich um mehrere Gr\"o{(\ss}enordnungen unterscheiden, ohne dass die Qualit\"at der Darstellung sich verschlechtert.
