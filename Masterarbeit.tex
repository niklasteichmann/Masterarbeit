...
\"Ahnlich wie beim \ref{section:Slicing} wird eine Transferfunktion verwendet, um den Voxeln eine Farbe zuzuordnen. Zus\"atzlich legt die Transferfunktion auch noch einen Wert im Intervall $[0,\dots 1]$ als Opazit\"at fest, die ausdr\"ckt, wie stark ein Voxel hinter ihm gerenderte Voxel \"uberdeckt (siehe Abb. \ref{Colormap} und die Beschreibung im Abschnitt \ref{section:Slicing}). Eine Opazit\"at von 1 bedeutet dabei, dass das Voxel komplett opak ist, eine Opazit\"at von 0 dagegen dass das Voxel vollst\"andig durchsichtig und damit in der Visualisierung unsichtbar ist. Wenn $T_O$, der Teil der Transferunktion $T$ der skalare Werte $s$ auf eine Opazit\"at abbildet, definiert ist als
...
so entspricht das erzeugte Bild einer Isof\"ache. Wenn dagegen nur eine zweidimensionale Schicht von Voxeln aus dem Datensatz dargestellt wird und die Opazit\"at konstant 1 ist, so wird eine Darstellung erzeugt, die dem Slicing \"ahnelt. Diese Optionen sind zwar in der Regel erheblich langsamer als auf die Darstellung von Isofl\"achen bzw. Slicings spezialisierte Algorithmen, zeigen jedoch die Flexibilit\"at des Direct Volume Renderings. 


<CONTINUE>

DVR Verfahren gliedern sich in 2 Gruppen, die als image-order und object-order bezeichent werden.

\begin{figure}
	\centering
	\includegraphics{pictures/DVR.png}
	\caption{Schematische Darstellung der image-order Verfahren. Farbwerte an Positionen auf der Bildebene werden durch Integration entlang von Geraden durch Kameraposition, der Position auf der Bildebene und dem Datensatz bestimmt.}
	\label{image-order}
\end{figure}

Allen image-order Verfahren ist gemein, dass sie eine Position im Raum als ``Kameraposition'' festlegen, die Position, von der aus der Benutzer den Datensatz betrachtet. Diese Position befindet sich dabei im gleichen Koordinatensystem wie die Voxel selbst. Vor der Kamera, orthogonal zur Blickrichtung, wird eine imagin\"are Ebene platziert, die sogenannte Bildebene (engl. ``image plane''). Ihre Gr\"o{\ss}e und Position ist so gew\"ahlt, dass die das gesamte Blickfeld der Kamera einnimmt (siehe Abb. \ref{image-order}). Indem Positionen der Bildebene auf Pixel des Bildschirms abgebildet werden, ist es m\"oglich, durch Einf\"arbung der Bildebene eine Visualisierung zu erzeugen. Die Farbe an einer Position der Bildebene ergibt sich dabei abh\"angig vom gew\"ahlten Verfahren \cite[S.~134]{kaufman2005overview}.

Im Gegensatz dazu zerlegen object-order Verfahren den volumetrischen Datensatz in eine Menge von Basiselementen oder Basisfunktionen, die auf eine Bildebene projeziert werden und damit den Datensatz darstellen\cite[S.~138]{kaufman2005overview}. Wenn sich Projektionen der Basiselemente \"uberlappen, was f\"ur nicht-degenerierte Datens\"atze praktisch der Standardfall ist, werden die Farbwerte abh\"angig von gew\"ahltem Verfahren und   

aus dem Integral des aus dem Datensatz kommenden Lichts entlang einer Linie, die die Kameraposition, die Position auf der Bildebene und Voxel des Datensatzes schneidet .
...
Aus diesem Modell wird ein Bild erzeugt, das den gesamten Datensatz visualisiert. Dazu wird 
