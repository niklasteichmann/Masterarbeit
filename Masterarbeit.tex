...
\begin{figure}
	...
	\caption{Darstellung der optimierten Abbildung von Voxeln auf Pixel. Wenn entweder die Voxel vollst\"andig transparent oder die Zielpixel vollst\"andig opak sind, werden sie \"ubersprungen (`skip'), ansonsten werden die Voxelwerte auf die Pixel abgebildet (`work'). Mit leichten Ver\"anderungen entnommen aus \cite{lacroute1994fast}.}
	\label{Scanline}
\end{figure}
...
Um in der Darstellung des Feldes im Invariantenraum Bereiche ausw\"ahlen zu k\"onnen, wurden Interaktionfl\"achen implementiert. Preim et al. \cite{preim2015interaktive} stellen an solche Elemente, die sie als ``3D Widgets'' bezeichnen, eine Reihe von Anforderungen:

\paragraph{1. Zerlegung von Interaktionen} Da Tastatur und Maus oft nicht gen\"ugend Freiheisgrade bieten um Interaktionen direkt duch sie umsetzen zu k\"onnen, wird die Interaktion in Teile zerlegt. F\"ur jeden Teil sollte ein Bereich des Widgets reserviert werden, der durch Maus und Tastatur manipuliert werden kann. Preim et al. bezeichnen diese Bereiche als ``Handles'' \cite[S.340]{preim2015interaktive}

\paragraph{2. Visuelle Unterscheidung vom Datensatz} Die Widgets m\"ussen klar von den dargestellten Daten zu unterscheiden sein, um falsche Interpretationen zu vermeiden.

\paragraph{3. Wahrnehmbare Eigenschaften} Die Darstellung des Widgets muss so gew\"ahlt werden, dass der Benutzer die Widgets als Interaktionsm\"oglichkeit wahrnehmen und ein Verst\"andniss f\"ur die Auswirkung der Interaktion entwickeln kann. Preim et al. bezeichnen dies als ``Affordances'' \cite[S.~340]{preim2015interaktive}.

\paragraph{4. Hervorherbung des selektierten Handles} Wenn der Benutzer ein Handle zur Interaktion selektiert hat, so sollte dieses hervorgehoben werden.

Innerhalb dieser Arbeit wurden zwei spezielle Interaktionen implementiert, durch die der Bereich der angezeigten Invarianten manipuliert werden kann. Beide Widgets werden in der Darstellung des Invariantenraumes angezeigt. Sie kombinieren die Konzepte des Cutaways \cite[S.~406f]{preim2015interaktive}, einer Interaktion bei der Teile des Datensatzes ausgeblendet werden um das Innere des Datensatzes zu zeigen, mit den Interaktionen zur Positionierung von Schnittebenen \cite[S.~401]{preim2015interaktive} (siehe dazu auch Abschnitt \ref{sec:Slicing}).

\subsubsection{Quadrisches Interaktionswidget}
Das erste Interaktionswidget hat die Form eines Quaders, der das erzeugte DVR vollst\"andig umfasst. Die Seitenfl\"achen sind parallel zu paarweise aus den Koordinatenachsen gebildeten Ebenen (xy-, xz-, yz-Ebene) und vollst\"andig transparent. Nur die Kanten des Quaders werden durch Linien dargestellt. Um die Orientierung des Quaders im dreidimensionalen Raum sichtbar zu machen, sind die drei Kanten, die zum Knoten links unten hinten zusammenlaufen eingef\"arbt: Die Kante parallel zur x Achse in rot, die Kante parallel zur y-Achse in gelb und die Kante parallel zur z-Achse in blau. Die Farben wurden so festgelegt, dass sie auch bei Farbenblindheit noch unterscheidbar sind. Die Positionen der Seitenfl\"achen entsprechen im Ausgangszustand den Minima und Maxima der einzelnen Invarianten. Die Geraden unterscheiden sich deutlich von der Darstellung des DVR, wodurch Anforderung 2 erf\"ullt wird.

Die Interaktion findet statt, indem eine Seitenfl\"ache durch Klicken und Halten der mittleren Maustasten ausgew\"ahlt wird und durch Bewegung der Maus entlang der zur Fl\"ache orthogonalen Achse verschoben wird. Jede Seitenfl\"ache stellt einen Handle dar, durch den das Minimum oder Maximum des angezeigten Intervalls einer einzelnen Invariante eingestellt wird. Dadurch ist Anforderung 1 erf\"ullt. Die Position der Seitenfl\"achen folgt dabei der Maus, so dass immer die aktuellen Intervallgrenzen gezeigt werden. Dadurch sind Anforderungen 3 und 4 erf\"ullt: Das aktuelle Handle liegt immer unter dem Mauszeiger und bewegt sich mit ihm, wodurch immer klar ist welches Handle ausgew\"ahlt ist. Und die Metapher der Interaktionsfl\"achen als Grenzen des angezeigten Intervalls erlauben dem Benutzer ein Verst\"andniss f\"ur die Interaktionen zu entwickeln.
...
Die Berechnung, ob ein Mausklick eine Interaktionsfl\"ache trifft, geschieht mittels `Ray-Picking'. Dabei wird, \"ahnlich wie beim Raycasting, ein Strahl ausgehend von der Kamera durch die Szene berechnet. Wenn der Strahl eine Interaktionsfl\"ache trifft, wird der getroffene Bereich ausgew\"ahlt.  Wenn mehr als eine Interaktionsfl\"ache getroffen wird, wird die n\"aheste bevorzugt. Preim et al. vergleichen diese Art der Auswahl von Widgets als Metapher zum Zeigen auf ein Objekt mit einem Laserpointer oder dem Greifen eines Objekts mit der Hand \cite[344]{preim2015interaktive}.
...
\begin{figure}
	...
	\end{subfigure}
	\hspace*{\fill}
	\caption{Die Darstellungen der zylindrischen Interaktionsfl\"achen. (a) Eine Interaktionsfl\"ache mit den einzelnen Bereichen. Ein Klick auf die Bereiche 1,2 und 3 w\"ahlen die jeweiligen Invarianten aus. Die Bereich 2a und 3a sind dabei die Minima der Invariante, 2b und 3b die Maxima. (b) bis (d) zeigen ver\"anderte Positionen und Formen der Interaktionsfl\"achen durch gew\"ahlte Invariantenbereiche. In (e) ist der Wireframe dargestellt.}
\end{figure}
